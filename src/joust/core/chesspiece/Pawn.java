...
public class Pawn extends ChessPiece {
	...
	public Pawn(Location l, Allegiance allegiance, int id) {
		this.location = l;
	...
	public ArrayList<Location> getPossibleMoves(Game g) {
		...
		//System.out.println("pawn possMoves myloc: " + location);
		Location l = null;
		...
		if(!isBlack()) {
			l = location.move(1, 0, 0, 0);
			moveUpLeft = location.move(1, 1, 0, 0);
			moveUpRight = location.move(1, 0, 0, 1);
			//System.out.println("pawn possMoves white: " + l + ", " + moveUpLeft + ", " + moveUpRight);
		} else {
			l = location.move(0, 0, 1, 0);
			moveUpLeft = location.move(0, 1, 1, 0);
			moveUpRight = location.move(0, 0, 1, 1);
		...
		if (l != null) {
			...
			if (checkAvailability(l).equals("unoccupied") && x < 9 && x > 0 && y < 9 && y > 0){
				...
				if (hasMovedAtAll == false) {
					if (!isBlack()) {
						Location l2 = location.move(2, 0, 0, 0);
						...
						Location l2 = location.move(0, 0, 2, 0);
						int x2 = l2.getXCoordinate();
						int y2 = l2.getYCoordinate();
						if (checkAvailability(g, l2).equals("unoccupied") && x2 < 9 && x2 > 0 && y2 < 9 && y2 > 0){
							possibles.add(l2);
		...
		if (moveUpLeft != null) { 
			...
			if (checkAvailability(g, moveUpLeft).equals("unoccupied") && hasMovedAtAll && ulX < 9 && ulX > 0 && ulY < 9 && ulY > 0) {
				Location moveLeft = location.move(0, 1, 0, 0);
		...
		if (moveUpRight != null) {
			...
			if (checkAvailability(g, moveUpRight).equals("unoccupied") && hasMovedAtAll && urX < 9 && urX > 0 && urY < 9 && urY > 0) {
				...
				Location moveRight = location.move(0, 0, 0, 1);
